...
function onResults(results) {
  ...
  // Pinch distance → brush size (samo kad nije u draw modu)
  if (!indexUp || middleUp) {
    const pinchDist = getDist(thumbTip, indexTip, W, H);
    // Map 20px–200px distance to brush size 2–40
    ...
    // Midpoint circle showing brush size
  ...
  // Draw landmark dots
  lm.forEach((point, i) => {
    const px = (1 - point.x) * W, py = point.y * H;
    overlayCtx.beginPath();
